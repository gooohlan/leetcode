...
func lowestCommonAncestor4(root *TreeNode, nodes []*TreeNode) *TreeNode {
	nodeMap := make(map[int]bool) // 将需要查找的节点放入map
	for _, node := range nodes {
		nodeMap[node.Val] = true
	}

	var find func(node *TreeNode, nodeMap map[int]bool) *TreeNode

	find = func(node *TreeNode, nodeMap map[int]bool) *TreeNode {
		if node == nil {
			return nil
		}

		if nodeMap[node.Val] { // 找到既返回
			return node
		}
