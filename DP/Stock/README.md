...
```
dfs[l][k][0] = res(dfs[l-1][k][0], dfs[l-1][k][1] + prices[l])
...
```
dfs[l][k][1] = res(dfs[l-1][k][1], dfs[l-1][k-1][0] - prices[l])
...
这个解释应该很清楚了，如果 `buy`，就要从利润中减去 `prices[l]`，如果 `sell`，就要给利润增加 `prices[l]`。今天的最大利润就是这两种可能选择中较大的那个。
...
```
dfs[-1][...][0] = 0
解释：因为 l 是从 0 开始的，所以 l = -1 意味着还没有开始，这时候的利润当然是 0。
